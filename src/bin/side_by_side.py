...
from src.saicinpainting.evaluation.data import PrecomputedInpaintingResultsDataset
from src.saicinpainting.evaluation.utils import load_yaml
from src.saicinpainting.training.visualizers.base import visualize_mask_and_images
...
def main(args):
    ...
    datasets = [
        PrecomputedInpaintingResultsDataset(
            args.datadir, cur_predictdir, **config.dataset_kwargs
        )
    ...
    for img_i in indices:
        try:
            ...
            vis_img = visualize_mask_and_images(
                cur_vis_dict,
                keys,
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        "--max-n", type=int, default=100, help="Maximum number of images to print"
    )
    aparser.add_argument(
        "--black", action="store_true", help="Whether to fill mask on GT with black"
    )
    aparser.add_argument(
    ...
    aparser.add_argument(
        "datadir", type=str, help="Path to folder with images and masks"
    )
    aparser.add_argument(
        "predictdirs", type=str, nargs="+", help="Path to folders with predicts"
    )
