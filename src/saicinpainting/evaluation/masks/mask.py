    ...
    def crop_to_canvas(self, vertical=True, horizontal=True, inplace=False):
        ...
        if horizontal:
            if self.left >= self.width or self.right <= 0:
                ...
                cut_left, cut_right = max(-self.left, 0), max(
                    self.right - self.width, 0
                )
                if cut_left != 0:
...
class SegmentationMask:
    ...
    def _augmentation_params(self):
        ...
        vertical_shift = np.random.uniform(
            -self.max_vertical_shift, self.max_vertical_shift
        )
    ...
    def _check_masks_intersection(self, aug_mask, total_mask_area, prev_masks):
        for existing_mask in prev_masks:
            ...
            intersection_current = (
                1 - (aug_mask.area() - intersection_area) / total_mask_area
            )
        ...
        for var_idx in range(self.num_variants_per_mask):
            ...
            augmentation_params["scaling_factor"] = min(
                [
                    ...
                    2
                    * min(orig_mask.up, orig_mask.height - orig_mask.down)
                    / orig_mask.height
                    + 1.0,
                    2
                    * min(orig_mask.left, orig_mask.width - orig_mask.right)
                    / orig_mask.width
                    + 1.0,
                ]
            )
            ...
            vertical_area = (
                aug_mask.mask.sum(axis=1) / total_aug_area
            )  # share of area taken by rows
            ...
            for horizontal_shift in allowed_shifts:
                ...
                if is_mask_suitable:
                    ...
                    augmentation_params["horizontal_shift"] = (
                        horizontal_shift / aug_mask_left.width
                    )
    ...
    def _prepare_mask(self, mask):
        height, width = mask.shape
        target_width = (
            width if self._is_power_of_two(width) else (1 << width.bit_length())
        )
        target_height = (
            height if self._is_power_of_two(height) else (1 << height.bit_length())
        )
    ...
    def get_masks(self, im, return_panoptic=False):
        ...
        for mask_id in potential_mask_ids:
            ...
            for params in masks_params:
                ...
                aug_mask.shift(
                    vertical=vertical_shift, horizontal=horizontal_shift, inplace=True
                )
...
def propose_random_square_crop(mask, min_overlap=0.5):
    ...
    if height < width:
        ...
        left_border = max(
            0,
            min(width - crop_size - 1, obj_left + obj_width * min_overlap - crop_size),
        )
        right_border = max(
            left_border + 1, min(width - crop_size, obj_left + obj_width * min_overlap)
        )
        ...
        top_border = max(
            0,
            min(height - crop_size - 1, obj_top + obj_height * min_overlap - crop_size),
        )
        bottom_border = max(
            top_border + 1, min(height - crop_size, obj_top + obj_height * min_overlap)
        )
