...
def main(args):
    ...
    if len(checkpoint_fnames) > 1:
        for fname in checkpoint_fnames[1:]:
            ...
            sum_tensors_cnt = 0
            other_cp = torch.load(
                os.path.join(args.indir, "models", fname), map_location="cpu"
            )
    ...
    shutil.copy2(
        os.path.join(args.indir, "config.yaml"),
...
if __name__ == "__main__":
    ...
    aparser.add_argument(
        ...
        type=str,
        default="last",
        help="Which checkpoint to take. " 'Can be "last" or integer - number of epoch',
    )
    aparser.add_argument(
        "--leave-discriminators",
        action="store_true",
