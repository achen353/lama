...
DEFAULT_CATS_FILE = os.path.join(
    os.path.dirname(__file__), "..", "configs", "places2-categories_157.txt"
)
...
def main(args):
    ...
    input_iterators = [
        iter(
            wds.Dataset(all_input_files[start : start + chunk_size]).shuffle(
                args.shuffle_buffer
            )
        )
    ...
    output_datasets = [
        wds.ShardWriter(args.outpattern.format(i)) for i in range(args.n_write_streams)
    ]
    ...
    while len(good_readers) > 0:
        ...
        if good_categories is not None and not is_good_key(
            sample["__key__"], good_categories
        ):
            bad_samples += 1
            continue
